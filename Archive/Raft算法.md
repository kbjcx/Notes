...
因此也需要采用两阶段协议来进行选举，加入一个 Pre-Vote 阶段，在发送选举消息前先询问整个系统，由收到消息的节点判断该候选者有无机会赢得选举，没有机会的话直接拒绝请求，避免了发起选举影响集群的可用性
跟随者判断候选者是否有机会的条件，即同意 Pre-Vote 的条件是：
1. 参数中的任期更大，或者任期相同但日志索引更大
2. 至少一次选举超时时间内没有收到领导者心跳

> [!tip] 
>  Pre-Vote 还可以解决由于网络分区导致的某个孤立
