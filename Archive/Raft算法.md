...
在从旧配置切换到新配置时，可能会出现矛盾的多数派，加入现在系统有三台服务器，现在管理员要添加两台新的服务器，但是新的配置无法同一时间在所有节点上切换，假设 S3 已经接受了新配置，S1 和 S2 还是旧的配置，那么就会在 S1、S2 上组成多数派，并且新配置上也会形成多数派。
...
Raft 算法通过联合共识 (Joint Consensus) 完成两阶段协议，即让新、旧两种配置都获得多数派选票

在第一阶段，领导者收到 $C_{new}$ 的配置变更请求后，先写入一条 $C_{old+new}$ 日志，配置变更立即生效。然后领导者将日志通过 Append Entries 消息复制到跟随者上，收到 $C_{old+new}$ 日志的节点立即应用该配置作为当前节点的配置；当 $C_{old+new}$ 日志被复制到多数派节点上时， $C_{old+new}$ 的日志就被领导者提交

$C_{old+new}$ 日志已提交保证了后续任何领导者一定保存了 $C_{old+new}$ ，领导者选举过程必须获得旧配置中的多数派和新配置中的多数派同时投票

$C_{old+new}$ 日志提交后，进入第二阶段，领导者立即写入一条 $C_{new}$ 日志，并将该日志通过消息复制到跟随者上，收到 $C_{new}$ 日志的跟随者立即应用该配置作为当前节点的配置，$C_{new}$ 日志复制到多数派节点，$C_{new}$ 日志被领导者提交，在 $C_{new}$ 日志提交以后，后续配置就都基于 $C_{new}$

但是还存在一个问题，当新的配置中不包含部分旧的节点时，由于在选举出领导者之后，不在 $C_{new}$ 中的节点收不到心跳包，如果没有及时关闭该进程，那么就会开启选举，他就有可能将新的选举消息发送给当前的领导者，由于任期号更大，那么就会导致领导者转化为跟随者状态，此时旧节点肯定无法完成选举，那么又会由包含 $C_{new}$ 的节点接任领导者，但是旧节点如果不下线的话仍然还会重复上述流程，虽然不影响系统日志的安全性，但是会导致系统可用性降低

因此也需要采用两阶段协议来进行选举，加入一个 Pre-Vote 阶段，**Pre-Vote 阶段称为预候选者，不会增加任期**，在发送选举消息前先询问整个系统，由收到消息的节点判断该候选者有无机会赢得选举，没有机会的话直接拒绝请求，避免了发起选举影响集群的可用性
跟随者判断候选者是否有机会的条件，即**同意 Pre-Vote 的条件是**：
1. 参数中的任期更大，或者任期相同但日志索引更大
2. 至少一次选举超时时间内没有收到领导者心跳

> [!tip] 
>  Pre-Vote 还可以解决由于网络分区导致的某个孤立节点频繁选举导致任期变大，在网络分区恢复时会发送选举消息影响领导者转化为跟随者，导致重新选举

#### 日志压缩
无论使用何种压缩算法，都有几个核心的共同点：
1. 不将压缩的任务集中在领导者之上，每个服务器独立的压缩其已提交的日志避免了领导者将日志传给已有该日志的模块
2. Raftsuanfa 要保留最后一条被丢弃的日志的索引和任期，用于一致性检查，同时，也要保存最新的配置信息，成员变更失败时需要回退配置
3. 一旦丢弃了前面部分的日志，领导者就要承担两个责任，如果服务器重启了就要将最新的快照加载到状态机后再接受日志；需要向较慢的跟随者发送一致的状态快照
